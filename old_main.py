...
from PyQt6.QtWidgets import QApplication, QTableView, QVBoxLayout, QWidget, QMessageBox, QTabWidget, QLabel, QHBoxLayout, QDialog, QDialogButtonBox
from PyQt6.QtGui import QPixmap
import ui_main  # Это наш конвертированный файл дизайна
import ui_pageAdd
...
class StationSelectionDialog(QDialog):
    ...
    def initUI(self):
        ...
        self.setStyleSheet('background-color: rgb(220, 228, 235); border: 1px solid rgba(0, 0, 0, 30); border-radius: 7px; color: black;')
...
class ResultDialog(QWidget):
    def __init__(self, images, values, plotMask, metricsMask):
        ...
        header_image = ['Кратчайшие пути', 'Сложность достижимости', 'Разделение зон ответственности']
        ...
        filtered_header_image = [elem for elem, m in zip(header_image, plotMask) if m]
...
class StationAddDialog(QtWidgets.QDialog, ui_pageAdd.Ui_Dialog):
    ...
    def get_data(self):
        return [self.stationName, self.lat_deg, self.lat_min, self.lat_sec, self.long_deg, self.long_min, self.long_sec, self.vel]

    def saveStation(self):
        errorFlag = self.check_input();
    ...
    def check_input(self):
        ...
        lat_deg = self.le_lat_deg.text()
        lat_min = self.le_lat_min.text()
        lat_sec = self.le_lat_sec.text()

        long_deg = self.le_long_deg.text()
        long_min = self.le_long_min.text()
        long_sec = self.le_long_sec.text()

        vel = self.le_stationSpeed.text()
        
        try:
            # Пытаемся конвертировать текст в float
            self.lat_deg1 = float(lat_deg)
            self.lat_min1 = float(lat_min)
            self.lat_sec1 = float(lat_sec)

            self.long_deg1 = float(long_deg)
            self.long_min1 = float(long_min)
            self.long_sec1 = float(long_sec)
...
class MainWindow(QtWidgets.QMainWindow, ui_main.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.pb_gen_graph.clicked.connect(self.show_error_message)
    ...
    def openResultWindow(self, plotMask, metricsMask, values):
        images = ["output/plot_example.png", "output/plot_reachability_example.png", "output/plot_reachability_difSt_example.png"]
        self.additional_window = ResultDialog(images, values, plotMask, metricsMask)
    ...
    def init_ui_main(self):
        ...
        self.le_avg_lifespan.setText('15')
    ...
    def startExperiment(self):
        ...
        if not errorFlag:
            plotMask = [self.cb_plot_closest_paths.isChecked(), self.cb_plot_map_reachability.isChecked(), self.cb_plot_map_difst_reachability.isChecked()]
            metricsMask = [self.cb_metr_avg_time.isChecked(), self.cb_metr_peak_time.isChecked(), self.cb_metr_avg_prob.isChecked(), self.cb_metr_peak_prob.isChecked()]


            if self.rb_method_line.isChecked():
                ptsFromPix, ptsToPix, ptsToMinTimes, plotData = expMakeLine(self.stationCoords, self.velocities, self.cases_number)
                values = [round(np.mean(ptsToMinTimes) * 60, 2), round(np.max(ptsToMinTimes) * 60, 2), round(probsFromTimes(np.max(ptsToMinTimes) * 60 + 1.5, 0.08).mean(), 2), round(probsFromTimes(ptsToMinTimes * 60 + 1.5, 0.08).mean(), 2)]
                ...
                if not errorFlag:
                    ...
                    #if self.mesh_width == 70 and self.check_freq == 40:
                    ...
                    ptsFromPix, ptsToPix, ptsToMinTimes, plotData = expMakeGraph(self.stationCoords, self.velocities, self.cases_number, G, Gmesh, Gmask)
                    values = [round(np.mean(ptsToMinTimes) * 60, 2), round(np.max(ptsToMinTimes) * 60, 2), round(probsFromTimes(np.max(ptsToMinTimes) * 60 + 1.5, 0.08).mean(), 2), round(probsFromTimes(ptsToMinTimes * 60 + 1.5, 0.08).mean(), 2)]
                    ...
                    if plotMask[1] or plotMask[2]:
                        ...
                        ptsToPix, ptsToMinTimes, plotData = ClosestPathWithTraceDijkstra(ptsFromPix, ptsToPix, self.velocities, G, Gmesh, Gmask)
    ...
    def check_input_SE(self):
        ...
        try:
            ...
            self.avg_lifespan = float(self.le_avg_lifespan.text())
    ...
    def updateFrameStyle(self):
        if self.graph_frame.isEnabled():
            self.graph_frame.setStyleSheet('background-color: rgba(255, 255, 255, 30); border: 1px solid rgba(0, 0, 0, 30); border-radius: 7px; color: black;')
        else:
            self.graph_frame.setStyleSheet('background-color: rgba(255, 255, 255, 30); border: 1px solid rgba(0, 0, 0, 30); border-radius: 7px; color: grey; opacity: 0.5;')
